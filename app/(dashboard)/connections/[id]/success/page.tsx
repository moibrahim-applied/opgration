...
export default function ConnectionSuccessPage({
  ...
  const apiEndpoint = connection
    ? `${baseUrl}/api/execute/${connection.organizationId}/${connection.projectId}/${params.id}`
    : `${baseUrl}/api/execute/org-id/project-id/${params.id}`;
...
{`curl -X POST ${apiEndpoint}/${exampleAction} \\
  ...
  -d '${examplePayload}'`}
            ...
            <div className="text-center pt-4 border-t">
              ...
              <code className="text-sm font-mono text-foreground">{params.id}</code>
            </div>
          </CardContent>
        </Card>
      </div>
